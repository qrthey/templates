...
(defn fill-out-template
  "Takes a template that can be any clojure datastructure and a
  replacement maps. A replacement map describes replacement values to
  create a value from the template. Keys in a replacement map must be
  strings.

  Returns a value that is an exact copy of the template, except for
  strings, keywords and pottentially symbols.

  Strings are transformed with clojure.string/replace for each
  key/value pair in the replacement map. The key in a pair is used as
